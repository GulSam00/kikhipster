...
> 갱신 시점: 2026-08-31 (§ Color 에 앨범 종류 배지 토큰 2종과 그 예외 추가)
...
| 랭킹 리스트 밀도 (국소) | Melon | 카탈로그 검증됨 | 톤(흰 배경)은 안 맞아 전체 채용 안 함. **밀도 논리는 2026-08-27에 철회했다** — 토너먼트 랭킹표에 적용해 뒀으나(커버 `size-10`, 제목 `text-sm`, 수치 `text-sm`+`text-xs`) 한 판의 결과를 훑기엔 너무 촘촘했다. 지금은 **커버 `size-16`, 제목 `text-lg`, 비율 `text-lg` 아래 분수 `text-sm`, 셀 `py-3`** 으로 한 행을 키운다. 멜론식 밀도는 "수백 곡을 스캔하는 차트"의 논리이고, 이 표는 후보 수십 개짜리 결과 화면이다 |
...
| `text-lg` `font-bold`/`font-medium` | 18px | 카드 제목, 서브섹션 헤더, 로고(Navbar), **토너먼트 랭킹 행의 제목·비율** |
| `text-sm` `font-normal`/`font-medium` | 14px | 본문, 리스트 아이템, 버튼 라벨, 카드 제목 |
...
| `album-ep` / `album-single` | **앨범 종류 배지 전용** — 다른 용도로 쓰지 않는다 | 저채도 (보라 / 청색) |
...
**예외 — 앨범 종류 배지 (2026-08-31 추가):** `AlbumTypeBadge` 의 `album-ep`(보라)·
`album-single`(청색)은 위 카운트에서 제외한다. 앞의 두 예외와 같은 논리다 — "주의를 끌기 위한
강조"가 아니라 **분류를 색으로 인코딩**하는 요소다. 단 (a) `--album-ep` / `--album-single`
토큰을 배경 15% · 테두리 30% 로 쓴 배지일 때만이고 (b) `album` 은 무채색을 유지해야 한다.
다른 용도(CTA·상태 표시)로 이 토큰을 가져다 쓰면 예외가 아니다.

> 이 예외를 넣은 경위: iTunes 는 앨범 종류를 필드로 주지 않고 제목 끝의 `- Single` / `- EP`
> 표기로만 알려 준다. 그 표기를 화면에서 떼기로 하면서(2026-08-31) **종류를 말하는 자리가
> 배지밖에 없어졌다.** 그런데 검색 결과는 카드가 20개 깔리는 화면이라 `primary`(amber)로
> 칠하면 즉시 BLOCK 이다. `primary` 를 CTA 자리로 남기려면 배지에 쓸 색이 따로 있어야 했고,
> 기존 토큰만으로는 3종을 가를 수 없어(saturated 가 `primary`·`destructive` 둘뿐) 토큰을
> 새로 뒀다. **색상환에서 amber(70°)와 멀리 두는 것이 조건이다** — 보라 293°, 청색 233°.
