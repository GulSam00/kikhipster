...
**현재 구현과의 격차** (2026-08-27 갱신): 1:1 매치업 카드는 이미 이 방향과 일치한다. 없는 건 "대진표 보기" 버튼과 그걸 눌렀을 때 뜨는 `Dialog` 기반 브래킷 트리(pan/zoom 포함)뿐이다. **트리가 필요로 하던 데이터 전제는 이미 해결됐다** — 월드컵 재설계로 대진과 라운드별 승자가 서버에 기록되므로(`/api/plays/{id}`) 새로고침에도 남는다. 예전에 여기 적혀 있던 "진행 상태가 `useState`에만 있어 새로고침하면 사라진다"는 더 이상 사실이 아니다. 착수 시점만 미정.
