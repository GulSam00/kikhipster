...
- [ ] **토너먼트가 새로고침에 소실된다.** 진행 상태는 React state에만, 트랙 메타는 클라이언트 `selected` 배열에만 있다. 이어하기를 지원하려면 ① 진행 중 토너먼트 목록/복구 엔드포인트 ② 트랙 ID로 메타를 되찾을 `GET /api/music/tracks/{id}` (또는 트랙 정보 DB 저장). `GET /api/tournaments/{id}` 는 이미 있으나 프론트가 안 쓴다. 브래킷 트리 뷰(`Dialog` 안 pan/zoom, `DESIGN.md` § Visual reference 참조)도 이 데이터가 있어야 착수 가능
