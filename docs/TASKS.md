...
- [ ] **토너먼트 — 기획 변경, 착수 가능 (T1 완료 2026-08-21).** 서버에 라운드별 진행 상태를 저장하고 복구 엔드포인트를 두는 기존 접근(`GET /api/tournaments/{id}` 하이드레이션 등)은 폐기. 새 방향:
  ...
  - `Tournament`/`TournamentRound` 모델의 `track_a_id`/`track_b_id`는 이제 iTunes 트랙 ID(숫자 문자열) 기준으로 설계
...
| **음악** | 아티스트/앨범/곡 검색, 아티스트 상세, 아티스트 앨범 목록, 아티스트 트랙 목록, 앨범 트랙 목록 — **iTunes Search API 기반, 인증 키 불필요**. 팔로워 수·인물 사진 없음(마이크 아이콘 폴백), "인기 트랙"은 진짜 인기순 아님(iTunes에 popularity 개념 자체가 없음) |
