...
### 2026-09-01 (계속) — 미결 결정 4건 처리 (코드 변경 없음)

과제 보드에 "정해야 한다"로 남아 있던 4건을 닫았다. **구현은 하나도 하지 않았다** —
결정만 기록하고 TASKS 에서 정리했다.

| 미결 | 결정 |
|------|------|
| 앨범 수록곡이 전곡을 보장하지 않음 (11곡 → 10곡) | **화면에 아무 표시도 안 한다.** 누락은 iTunes 쪽 사정이고, 안내 문구가 오히려 신뢰를 떨어뜨린다고 봤다 |
| 아티스트 앨범 목록의 중복 (`NJWMX` 두 행) | **그대로 둔다.** iTunes 가 실제로 두 발매본을 들고 있는 것이라 우리 버그가 아니고, 접으면 리마스터·디럭스까지 잘못 접힐 위험이 생긴다 |
| 리뷰 vs 감성 태그 | **계속 보류.** 브라우저 검증 12건과 테스트·CI 가 먼저다 |
| 아티스트 인물 사진 | **방향만 정하고 착수는 안 한다** — 아래 |

**아티스트 인물 사진 — 실측 조사 후 방향만 확정.** 후보 API 를 실제로 호출해 비교했고,
그 결과 **출처 Deezer 단독 · 저장은 원본 URL 참조 · 적용은 아티스트 상세만** 으로 정했다.
자체 호스팅(S3+CloudFront)은 하더라도 T4 와 함께 본다. **화면이 `image_url` 문자열
하나만 보므로 저장 계층은 나중에 갈아끼워도 되돌리는 비용이 없다** — 그래서 지금
더 정하지 않았다. 실측 근거는 `docs/TASKS.md` "언젠가" 항목에 남겼다. 요지만 옮기면:

- **2026-08-21 에 Deezer 를 탈락시킨 사유는 이 용도에 해당하지 않는다.** 그때 문제는
  *혼합 쿼리*(`"아이유 Blueming"` 0건)였고, 인물 사진은 **이름 단독 조회**라 다른 경로다.
  실측에서 `아이유`→IU, `뉴진스`→NewJeans, `米津玄師`→Kenshi Yonezu 전부 정확했다
- **1위가 정답이 아닐 수 있다** — `김광석` 은 1위가 `nb_fan=30` 유령 항목, 진짜는 2위였다
- **'사진 없음'이 200 + 회색 이미지로 위장해서 온다** — URL 해시가 비었거나
  `d41d8cd98f00b204e9800998ecf8427e`(빈 문자열의 MD5)면 없는 것. **URL 만 보고 걸러진다**
- MusicBrainz→Wikidata→Commons 도 되지만 1req/s 를 지켜도 버스트에 503 을 맞았다

**프론트 변경 비용은 거의 없다는 것도 확인했다** — `CoverImage` 가 이미 `src` 가 없으면
마이크 아이콘을 그리므로, 값이 채워지면 그냥 사진이 뜬다. `next.config.ts` 의
`remotePatterns` 한 줄만 늘어난다.

### 2026-09-01 (계속) — 화면 정리 5건

설계 `_workspace/01_architect_spec.md`, 검증 `_workspace/qa_report.md`.

**① 상세 액션 줄을 세로 2단 중앙으로.** `justify-between` 이라 넓은 화면에서 주요 동작과
좋아요·공유가 화면 양 끝까지 벌어져 한 줄로 안 읽혔다. `DetailActionBar` 한 곳만 고치면
탑스터·월드컵이 같이 바뀐다. 주요 버튼은 `h-11` → `h-12`.
**`PlayLauncher` 는 `data-[size=default]:h-11` 도 같이 `h-12` 로 바꿔야 했다** —
variant 를 맞춰 덮지 않으면 기본값이 이기는, CLAUDE.md 에 적힌 그 함정이다.

**② 볼륨을 컨트롤 줄 오른쪽 끝으로.** 2026-08-30 에 가로 스크롤 때문에 위 줄로 뺐던 것을
되돌리되 제약은 지켰다 — **묶음 전체를 `sm` 이상에서만** 보이게 하고, 좁은 화면에 음소거
수단이 사라지지 않도록 **위 줄에 음소거 버튼만 `sm:hidden` 으로 남겼다.**

**③ 곡 행 전체가 재생 버튼이 됐다.** `<div onClick>` 이 아니라 `absolute inset-0` 로 덮는
진짜 `<button>` 을 형제로 깔았다 — `AlbumCard` 가 링크로 쓰는 패턴이다. div 로 만들면
키보드 접근과 focus ring 을 직접 붙여야 하고(§ Component states — 없으면 BLOCK), 버튼 안에
버튼은 무효 마크업이라 하트를 자식으로 둘 수 없다. **미리듣기가 없는 곡은 덮개를 아예
렌더하지 않는다.** 재생 버튼은 행 맨 앞(번호 앞)으로 옮기고, 덮개와 동작이 같으므로
`aria-hidden`·`tabIndex={-1}` 로 중복 노출을 없앴다.

**④ 검색 이미지 — 원인이 둘로 갈렸다.**

| 대상 | 원인 | 조치 |
|------|------|------|
| 곡 | 커버는 **정상적으로 온다**. `TrackRow` 가 받고도 안 그렸다 | 고침 |
| 아티스트 | iTunes 아티스트 엔티티에 **이미지 필드가 없다** | 불가 |

곡 쪽은 API 응답에 `album.cover_url` 이 멀쩡히 있는데 화면이 쓰지 않던 것이라 `showCover`
하나로 끝났다. **아티스트는 데이터 자체가 없어서 UI 로는 못 고친다** — `/lookup?entity=album`
으로 최신 앨범 커버를 대신 쓰는 우회로가 있지만 2026-08-20 에 "앨범 위주 도메인이라 인물
사진은 필수 아니다"로 채택하지 않은 판단이 있고, 아티스트 1명당 요청 1회가 더 나가
(검색 20명이면 20회, 배치 불가) 캐시가 필요하다. 이번엔 보고만 했다.

**곁가지로 버그 하나를 찾았다** — 아티스트 상세가 `albumCover={artist.image_url}` 을
넘기고 있었다. **그 값은 항상 null 이므로** 그 페이지에서 재생을 시작하면 하단 재생기의
커버가 늘 비어 있었다. `t.album.cover_url` 로 고쳤다.

**⑤ 월드컵 종류를 카드와 상세 양쪽에.** 썸네일과 후보 그리드가 둘 다 앨범 아트라 그림만으로는
곡 월드컵인지 앨범 월드컵인지 알 수 없다. **`ITEM_TYPE_LABEL` 이 그동안 OG 설명에만 쓰이고
화면에는 없었다.** 2026-08-27 에 카드에서 뺀 것은 *수량*('앨범 N', `ItemStats` 와 겹쳤다)이고
이번에 넣은 것은 *종류*라 겹치는 정보가 없다. 상세는 탑스터가 격자 크기를 넣는
`DetailHeader` `badges` 자리를 쓴다. 둘 다 무채색이다 — 카드가 여러 장 깔리는 화면이라
색을 쓰면 § Color budget 에 걸린다.

**확인**: `tsc` 0 · eslint 신규 0 · `pnpm build` 통과. SSR 로 앨범 상세(덮개 12개, 자식 순서
`덮개 → 재생버튼 → …`), 아티스트 상세(커버 10개), 월드컵 상세(`앨범 월드컵` 배지,
`flex-col items-center`, `h-12`)를 확인했다. **볼륨·카드 배지·탑스터 액션 줄·행 클릭 동작은
클라이언트 렌더라 못 봤다**(TASKS 에 남김).
...
남은 것은 `AlbumCard`·`ArtistCard` 의 **카드 내부 여백**뿐이다.

| 시도 | 결과 |
|------|------|
| 그리드 간격 `gap-3` → `gap-4 sm:gap-6` | **되돌림.** 카드 사이가 아니라 카드 안 이야기였다 |
| 열 수 `sm:4 md:5` → `sm:3 md:4` | **되돌림.** 한 줄 5개 유지가 낫다는 판단 |
| **카드 여백 `size="sm"` → 기본값** | **유지** |
| 글자 `text-sm`/`text-xs` → `text-base`/`text-sm` | **되돌림.** 5열에서는 폭이 안 나온다 |

**남긴 변경**: `size="sm"`(`--card-spacing` 12px)을 기본값(16px)으로 올리고 `gap-2` 로 덮던
것을 지웠다. **패딩과 세로 간격이 모두 `--card-spacing` 하나에서 나오므로** 이 한 곳만
바꾸면 카드 안이 고르게 넓어진다 — 개별 클래스로 맞추면 프리미티브가 바뀔 때 어긋난다.
거기에 `CardContent` `gap-2` → `gap-3`, 아티스트명 `mt-0.5`, 배지 줄 `mt-2` 를 더해
커버 / 제목·아티스트 / 배지·메타 세 덩어리를 갈랐다.

**글자를 되돌린 이유는 폭이다.** 한 줄 5개면 콘텐츠 폭이 데스크톱 약 186px · 태블릿 약
150px · 모바일 2열 약 106px 다. 16px 제목은 각각 한글 11·9·6자에서 잘려서
`NewJeans 2nd EP 'Get Up'` 같은 제목이 데스크톱에서도 끊긴다. **글자를 키우려면 열을
줄여야 하는데 그건 안 하기로 했으므로 14px/12px 가 맞다.** DESIGN.md § Typography 에
잠깐 넣었던 `text-base` 행도 지웠다 — 실사용이 없는 값이 표에 남으면 안 된다.

**`ArtistCard` 를 매번 같이 바꿨다.** 검색에서 두 카드가 같은 자리에 탭으로 교체되므로
한쪽만 손대면 탭을 옮길 때 카드가 튄다.

**확인**: `tsc` 0 · eslint 0(`search/page.tsx:80` 의 `set-state-in-effect` 는 기존 것) ·
SSR 에서 그리드 `gap-3 sm:grid-cols-4 md:grid-cols-5`, 카드 `data-size="default"`,
`CardContent` `gap-3`, 제목 `text-sm` / 아티스트 `text-xs` 확인.

### 2026-08-31 (계속) — 앨범 종류 배지 (색 3종), DESIGN.md 색 규약 개정

제목에서 꼬리를 뗀 결과 **종류를 말하는 자리가 배지밖에 없어졌다.** 검색 미리보기에도
ALBUM / EP / SINGLE 을 색으로 갈라 달라는 요청이 이어졌다.

**§ Color budget 과 정면으로 부딪혔다.** saturated 토큰이 `primary`(amber)와 `destructive`
둘뿐이라 3종을 색으로 가를 수 없고, 검색 앨범 탭은 **카드가 20개 깔리는 화면**이라
배지에 `primary` 를 쓰면 "한 화면 primary 4개 초과 = BLOCK"에 즉시 걸린다.
세 안(토큰 신설 / SINGLE·EP만 색 / 무채색 3단계)을 놓고 물었고 **토큰 신설**로 정했다.

`--album-ep`(보라 293°) · `--album-single`(청색 233°) 를 `globals.css` 에 라이트·다크 2벌로
두고, DESIGN.md § Color 에 토큰과 예외를 명문화했다 — 2026-08-27 체크박스 예외와 같은
방식이다. **amber(70°)와 색상환에서 멀리 두는 것이 조건**이고, 배경 15% · 테두리 30% 로
채도를 낮춰 커버 아트를 이기지 않게 했다. `album` 은 가장 흔한 기본값이라 무채색으로 남겼다.

**배지는 제목 옆이 아니라 메타 줄에 뒀다.** 제목 옆이면 모바일 2열(카드 폭 150px 남짓)에서
제목이 배지에 밀려 두세 글자만 남는다 — 종류보다 제목이 먼저다.

**CSS 규칙이 실제로 생성된 것까지 확인했다.** 프로덕션 CSS 에 토큰 2벌(+`lab()` 폴백)과
`.bg-album-ep\/15{...color-mix(in oklab, var(--album-ep) 15%, transparent)}` 같은 유틸리티가
들어갔다. `/artists/1635469693` SSR 에서 `EP` 3 · `Single` 11 · `Album` 2 로 갈린다.

**dev 서버가 `globals.css` 변경을 안 잡아 한동안 토큰이 CSS 에 0건으로 보였다.**
Turbopack 캐시(`.next/dev`)를 지우고 재기동해야 반영된다 — CSS 토큰을 추가한 뒤 화면이
그대로면 **코드가 아니라 캐시를 먼저 의심할 것.**

**DESIGN.md 를 고치다 기존 문단을 한 번 깨뜨렸다.** 체크박스 예외의 경위 인용문 앞부분을
앵커로 잡아 치환하는 바람에 뒷부분이 고아가 됐고, 원문을 되살려 제자리로 옮겼다.
긴 문단을 앵커로 쓸 때는 **문단 전체**를 잡아야 한다.

### 2026-08-31 — 앨범 제목 꼬리 제거, 싱글·EP 필터 해제 (2026-08-23 해석 오류 정정)

설계 `_workspace/01_architect_spec.md`, 검증 `_workspace/qa_report.md`.
이전 세션 산출물은 `_workspace/archive/2026-08-30/` 로 옮겼다(기존 `_workspace_prev/` 에
`planning.md` 가 있어서 규약대로 덮어쓰면 잃는다).

**2026-08-23 에 요청을 잘못 옮겼다.** 당시 요청은 "제목 뒤에 붙은 `- Single` / `- EP` 가
거추장스럽다"였는데 구현은 **"그런 앨범을 결과에서 빼자"** 로 갔다. 제목을 가공하는 코드는
그때도 지금도 없었다(`_map_album` 은 `collectionName` 을 원문 그대로 넘긴다). 그래서:

- 필터가 걸린 두 경로에서는 항목 자체가 없어서 표기도 안 보였다 — **증상만 가려졌다**
- 필터가 없는 경로(`/albums?ids=`, `/albums/{id}/tracks`)에서는 꼬리가 그대로 보였다 —
  탑스터 격자 옆 목록, 탑스터 PNG, 월드컵 후보·대결·랭킹, 앨범 상세
- 그러면서 `NewJeans 2nd EP 'Get Up'` 같은 **정규 미니앨범이 통째로 사라졌다**.
  iTunes 가 K-POP 미니앨범을 `- EP` 로 표기하기 때문이다 — newjeans 검색이 8건만 나오던 이유

**세 갈래로 정리했다.**

1. **꼬리 제거는 프론트에서.** `lib/domain/album-title.ts` 의 `stripAlbumSuffix` 를
   `lib/api/music.ts` 가 부른다. **백엔드에서 안 한 이유는 캐시** — `music_cache` 는 매핑을
   마친 값을 저장해서(`_warm_item_cache`) 거기서 떼면 기존 행이 최대 30일간 옛 제목을
   내보낸다. 한 화면에 정리된 제목과 안 된 제목이 섞인다.
   **`lib/api/music.ts` 한 곳에 건 것이 핵심이다** — 앨범이 프론트로 들어오는 유일한 문이라
   검색·상세는 물론 탑스터 PNG 와 월드컵 후보까지 한 번에 따라온다. 화면마다 떼면 반드시
   빠뜨린다.
2. **필터 기본값을 뒤집었다.** `include_singles` 를 `False` → `True`. 파라미터는 남겼다 —
   나중에 "싱글 숨기기" 토글을 붙일 자리다. 오버페치(`limit*3`)는 코드를 안 고쳐도 자동으로
   꺼진다(`if not include_singles`).
3. **`album_type` 에 중간 ` EP ` 규칙을 더했다.** 꼬리 → 중간 EP → 트랙 수 순으로 판정한다.

**판정과 필터의 규칙을 일부러 다르게 뒀다.** `is_single_or_ep`(필터)는 꼬리만 보고
`_album_type`(판정)은 중간 EP까지 본다. 필터를 켠 쪽이 기대하는 것은 "iTunes 가 꼬리로
분류한 것"이고, 중간 EP까지 거르면 이번에 살리려던 미니앨범을 다시 잃는다.

**중간 EP 는 제목에서 떼지 않는다.** 꼬리는 iTunes 가 덧붙인 분류표라 떼도 이름이 남지만
`NewJeans 2nd EP 'Get Up'` 의 EP 는 이름의 일부다 — 떼면 `NewJeans 2nd 'Get Up'` 이 된다.

**규칙을 추측으로 넓히지 않았다 — 4206건 실측.** 25개 질의(K-POP·해외)로 모은 고유 앨범에서
중간 ` EP ` 규칙이 새로 잡는 것은 **3건뿐이고 전부 정탐**이었다(NewJeans 1st/2nd EP,
Karaoke Piano EP). **오탐 0건.** 소문자 ` ep ` 는 4206건 중 0건이라 대소문자 무시로
완화하지 않았다 — 얻는 게 없으면서 `Deep`/`Sleep` 위험만 커진다. 양쪽 공백도 같은 이유로 필수다.

**부수로 하나 고쳤다: 배지의 `capitalize` 가 `ep` 를 `Ep` 로 렌더했다.** EP 는 Extended Play
약어라 두 글자를 다 올려야 한다. 필터 때문에 `Album` 외의 값이 화면에 거의 안 떠서 그동안
드러나지 않았고, **필터를 푸는 이번 변경으로 처음 보이게 됐다** — SSR HTML 에서 실제로
`Ep` 인 것을 보고 `albumTypeLabel()` 로 교체했다.

**검증**: 판정 12케이스 통과 · 4206건 실측 오탐 0 · `tsc` 0 · eslint 0 · `next build` 통과.
**스택을 실제로 띄워 확인했다** — `/albums/{id}` 세 건에서 제목이 `Zero (J.I.D Remix)` 로
정리되고 배지가 `Single`/`EP`/`Album` 으로 갈리는 것, `/artists/1635469693` 전체 HTML
206KB 에서 **`- Single`·`- EP` 잔여 0건**이면서 싱글·EP 앨범은 정상 노출되는 것까지.
`/search`·PNG·월드컵 후보는 클라이언트 렌더라 못 봤다(TASKS 에 남김).

**교훈**: 요청의 증상("꼬리가 거추장스럽다")과 해법("그 항목을 빼자")을 바꿔치기하면
증상이 가려진 채 원인이 남고, 그 해법이 **다른 것까지 지운다**. 2026-08-30 의
"뒤 배경에" 오독과 같은 종류의 실수다 — 사용자가 말한 단어를 그대로 읽어야 한다.

### 2026-08-30 (계속) — 반동이 잘리던 문제 (클립 상자 넓히기)

`windup` 을 키웠더니 반동이 화면 밖으로 나가 안 보인다는 지적. 원인은 진 카드의 가로
스크롤을 막으려고 넣었던 `overflow-x-clip` 래퍼였다 — **잘리는 선이 격자와 정확히 같은
폭**이라, 카드가 칸을 꽉 채우고 있는 상태에서 바깥으로 빼는 순간 족족 잘렸다.

잘리는 선만 바깥으로 민다. 레이아웃은 건드리지 않는다.

1. **`-mx-4 px-4`** — 래퍼를 페이지 컨테이너의 좌우 여백(`px-4`)만큼 넓히고, 안쪽 패딩으로
   격자를 제자리에 돌려놓는다. `overflow: clip` 의 기준은 **패딩 상자**라 패딩 안쪽은 안
   잘린다 → 좌우 16px 씩 여유. **`-mx-4` 가 안전한 최대치다** — 이보다 키우면 래퍼가
   컨테이너 바깥으로 나가서 좁은 화면에 진짜 가로 스크롤이 생긴다(BLOCK).
2. **`[overflow-clip-margin:1.5rem]`** — 거기서 24px 더 민다. 미지원 브라우저는 무시하고
   ①의 16px 만 남으므로 깨지지 않는다.

**반동이 안 잘리는 한계는 약 40px**(미지원 브라우저 16px). `windup` 상한을 그보다 크게
잡으면 다시 잘린다.

더 필요하면 클립을 **뷰포트 폭**에서 하는 방법이 있다(`layout.tsx` 의 `main` 에
`overflow-x-clip`). 그러면 반동 한계가 사실상 사라지지만 모든 페이지에 영향을 주므로
지금은 안 했다.

### 2026-08-30 (계속) — 승리 카드에 반동(anticipation) 추가

중앙으로 출발하기 전에 **반대쪽으로 잠깐 뺐다가** 간다. 치기 전에 뒤로 당기는 동작이라
이게 있어야 "밀어냈다" 로 읽힌다 — 없으면 그냥 미끄러지는 것으로 보인다.

**이 구간만 스프링이 아니라 키프레임으로 직접 그렸다.** 스프링은 목표를 지나쳤다 돌아오는
follow-through 는 저절로 만들지만 **출발 전에 반대로 빼는 것은 만들지 않는다.** 초기 속도를
음수로 주면 비슷해지지만, 그러면 `stiffness`/`damping` 물리 스프링이 되어 **수렴 시간이 열려
있다** — 투표 핸들러가 680ms 뒤에 판을 넘기므로 끝나는 시각이 정해져 있어야 한다.
`x` 만 키프레임(`[0, -dir*windup, toCenter]`)으로 가고 `scale` 은 스프링 그대로 뒀다.

두 번째 구간의 베지어 `[0.22, 1.1, 0.36, 1]` 은 y2 가 1 을 넘어 **중앙을 지나쳐 상대 쪽으로
더 갔다가** 돌아온다 — 스프링의 `bounce` 가 하던 일을 그대로 한다.

빼는 거리는 `min(|중앙까지 거리| * 0.12, 18px)`. 거리에 비례하되 좁은 화면에서 과하지 않게
상한을 뒀다.

**진 카드의 출발을 0.07초 → 0.2초로 미뤘다.** 뒤로 뺐다 되돌아온 이긴 카드가 **실제로 닿는
시점**에 맞춰야 맞아서 밀리는 것으로 읽힌다. 타이밍은 이긴 카드 0.62초, 진 카드 0.2+0.45=
0.65초로 대기 0.68초 안에 둘 다 들어간다.

### 2026-08-30 (계속) — 배경 대진표가 만든 잔상 (앞선 진단 두 번 틀림)

**"밀려나는 카드 뒤로 이긴 카드가 비친다"는 지적의 진짜 원인은 `BracketBackground` 였다.**
앞서 두 번 고친 것(진 카드 `opacity` 를 72% 까지 유지, 진 카드에 `z-20`)은 **엉뚱한 곳이었다.**
카드끼리의 겹침 문제로 읽고 카드만 봤는데, 사용자가 처음부터 "**뒤 배경에**" 라고 했다.

`BracketBackground` 는 `justPicked` 가 서면 오른쪽 '다음 자리' 상자를 **이긴 항목의 제목**으로
바꾸고 `border-primary text-primary opacity-100` 으로 밝힌다. 그 자리가 하필 **이긴 카드가
도착하는 자리**라, 움직이는 카드 뒤로 그 카드의 제목이 비치는 것처럼 보였다.

**고르는 순간에는 이 층 전체를 걷는다.** 예전엔 "선택한 쪽이 다음 자리로 올라간다"를 배경이
말했는데, **이긴 카드가 직접 상대를 튕겨 내고 중앙으로 오는 연출이 생기면서 같은 말을 두 번
하게 됐다.** 승자가 어디로 가는지는 이제 카드 자신이 말한다. 걷는 것은 그 순간뿐이고 다음
경기가 올라오면 트리는 그대로 돌아온다.

배경이 안 보이는 동안만 도는 분기(진 쪽 상자 흐리기, 다음 자리에 승자 제목 채우기)는 죽은
코드라 같이 지웠다. `justPicked` 는 이제 **걷을지 말지에만** 쓰인다.

**DESIGN.md 와의 관계**: § Visual reference 는 배경 트리에 대해 "'선택한 쪽이 올라가는'
애니메이션도 이 범위에서 가장 잘 읽힌다"고 적어 뒀다. 그 역할이 배경에서 **카드로 옮겨 간
것**이므로 트리를 상시로 까는 방침 자체는 그대로다.

**교훈**: 증상 설명의 단어를 그대로 읽었어야 했다. "뒤 배경에"는 카드 뒤가 아니라 배경 층을
가리키는 말이었고, 두 번의 수정이 그걸 놓쳐서 헛돌았다.

### 2026-08-30 (계속) — 튕겨내기를 `motion` 스프링으로 교체

CSS keyframes 판을 `backup/2026-08-30-battle-css-keyframes/` 에 통째로 복사해 두고 갈아탔다
(되돌리는 방법은 그 폴더 README 에).

**왜 스프링인가.** keyframes 는 정해진 궤적을 재생할 뿐이라 부딪히는 순간의 반동이 손으로
찍은 값(18%/34% 같은 것)이었다. 스프링은 **목표를 지나쳤다가 되돌아오는 것이 계산 결과**다.
이긴 카드의 목표는 격자 중앙이고 상대는 그 너머에 있으므로, `bounce` 로 생기는 오버슛이
**그대로 상대 쪽으로 밀고 들어가는 동작이 된다.** 초기 속도를 따로 줄 필요가 없었다.

**물리 엔진(matter-js 등)을 안 쓴 이유는 그대로다** — 이 연출은 이긴 카드가 **정확히**
가운데에 서서 끝나야 하는데 강체 시뮬레이션은 정해진 좌표에 서지 않는다. 스프링은 목표값으로
**수렴**하므로 그 요구를 지키면서 관성감만 가져온다. 새 의존성도 0이다(`motion` 은
`SplitFlapText` 때 이미 들어와 있다).

**`duration` + `bounce` 로 준 이유.** `stiffness`/`damping` 으로 주면 수렴 시간이 열려 있어
**다음 경기가 애니메이션 도중에 끼어들 수 있다**(투표 핸들러가 680ms 뒤에 판을 넘긴다).
`duration` 기반 스프링은 끝나는 시점이 정해져 있어 그 안에 확실히 들어간다.

**이동 거리를 CSS 가 아니라 실측으로 구한다.** `getBoundingClientRect()` 로 두 카드의 중심을
재고 **그 중점이 곧 격자의 중앙**이다. 간격을 알 필요가 없어져서 `--battle-gap` 변수와
`gap-(--battle-gap)` 을 없애고 평범한 `gap-3 sm:gap-6` 으로 되돌렸다.

**두 군데 함정이 있었다.**

1. **애니메이션을 카드가 아니라 래퍼에 건다.** 결승에서는 카드가 `StarBorder` 안으로 들어가
   격자 칸의 주인이 바뀐다. 래퍼가 항상 칸의 주인이면 두 경우를 같은 코드로 다룰 수 있고,
   z-index 도 래퍼에 걸어야 칸끼리 순서가 잡힌다.
2. **`key` 에 경기 id 를 섞어야 한다.** 이긴 항목은 **다음 경기에도 나오므로** `key` 가 항목
   id 뿐이면 React 가 같은 DOM 노드를 재사용하는데, 그 노드에는 `motion` 이 남긴 인라인
   transform(가운데로 옮겨 놓은 값)이 그대로 붙어 있다. 경기가 바뀌면 새 노드가 되게 했다.

`animate()` 를 transform 용과 opacity 용으로 나눠 부른다. 한 번에 부르면서 `velocity` 를
주면 `scale` 에까지 그 속도가 걸린다.

앞서 고친 두 가지(진 카드가 72% 까지 불투명, 진 카드가 위에 그려짐)는 스프링 판에도 그대로
옮겼다.

**확인**: `tsc`·`pnpm build`·eslint 통과(error 2건은 기존 것), 빌드 CSS 에서 `battle-*`
keyframes 0건 + `star-movement` 는 유지, `/tournament/{id}` 200. **움직임 자체는 여전히
눈으로만 판정된다.**

### 2026-08-30 (계속) — 튕겨내기 겹침 수정, 물리 엔진 조사

**버그**: 밀려나는 카드 뒤로 이긴 카드의 내용이 비쳐 보인다는 지적. 원인이 둘이었다.

1. **진 카드가 비행 내내 반투명이었다.** `opacity` 를 16% → 100% 로 서서히 지워서 날아가는
   구간 대부분이 반투명한 창이 됐다. → **72% 까지 `opacity: 1` 로 버티게** 바꿨다.
2. **가리는 순서가 반대였다.** 이긴 쪽에만 `z-10` 을 줬는데 positioned 요소는 static 요소
   위에 그려지므로, 겹치는 구간에서 **이긴 카드가 진 카드를 뚫고 나왔다.** → 튕겨 나가는
   쪽이 앞으로 날아가는 것이므로 진 카드에 `z-20`, 이긴 카드에 `z-10`.

겹침 자체도 줄였다 — 이긴 카드가 34% 까지 자기 자리 근처에서 버티다가 중앙으로 간다.
곧바로 출발하면 아직 날아가는 중인 상대와 크게 겹친다.

**물리 엔진 조사 (앞서 안 찾아본 것에 대한 보충).**

- **matter-js** — 2D 강체 엔진. **React Bits `FallingText` 가 실제로 쓴다.** v0.20.0,
  unpacked 927KB.
- **@react-three/rapier** — Rust/WASM. three.js 장면용이라 DOM 카드에는 안 맞는다.
- **p2-es** — matter-js 계열, 사용처가 적다.
- React Bits `Ballpit`(three+gsap)은 볼풀 데모, `Magnet`(무의존)은 커서 자석이라 충돌이 아니다.

**안 쓰기로 한 이유**: 이 연출은 **이긴 카드가 정확히 가운데에 서서 끝나야** 하고 곧바로
다음 경기가 로드된다. 강체 시뮬레이션은 정해진 좌표에 안 선다 — 결과를 덮어써야 하는데
그러면 엔진을 쓰는 의미가 없다. 다만 **진 카드의 퇴장은 결정성이 필요 없어서**(그냥 나가면
된다) 거기만 물리로 굴리는 절충은 가능하다.

**더 나은 중간 선택지가 있다**: `motion` 이 **이미 설치돼 있다**(`SplitFlapText` 작업 때
들어왔다). 스프링은 목표값으로 수렴하므로 **정확히 가운데에 서는 것을 지키면서** 관성감을
낼 수 있다. 새 의존성 0. 지금은 CSS 를 유지하고 이 선택지를 남겨 둔다.

### 2026-08-30 (계속) — 튕겨내기 연출, 결과 화면 댓글, 비로그인 댓글·신고

요청 3건을 한 벌로 했다. 설계는 `_workspace/01_architect_spec.md`, 검증은
`_workspace/qa_report.md`.

**① 이긴 카드가 진 카드를 튕겨 낸다.**

새 의존성 없이 CSS keyframes 로 했다(`motion` 이 이미 있지만 레이아웃 밖으로 날리는 단순
트윈에 라이브러리를 쓸 이유가 없다).

- **방향을 `--battle-dir`(1 / -1) 한 변수로 넘겨** 좌·우 keyframes 를 네 벌이 아니라 두 벌로
  끝냈다. 부호는 **이긴 카드가 어느 칸에 있었는지**로 정한다 — 두 카드가 같은 부호를 써야
  한쪽이 다른 쪽을 미는 것처럼 보인다.
- 가운데로 가는 거리는 `calc(50% + var(--battle-gap) / 2)`. `translateX` 의 `%` 는 자기 폭
  기준이라 **자기 폭 절반 + 간격 절반**이 정확히 2열 격자의 중앙이다. 간격이 반응형이자
  라운드별로 달라져서 유틸리티가 아니라 **변수로 들고**(`gap-(--battle-gap)`) keyframes 에서
  읽는다.
- 18% 지점이 부딪히는 순간이다. 이긴 카드는 거기까지 상대 쪽으로 밀고 들어갔다 중앙으로
  가고, 진 카드는 그때까지 버티다 날아간다.
- **가로 스크롤을 막는 게 이 작업에서 제일 신경 쓴 부분이다**(§ Mobile, BLOCK). 격자를 감싼
  상자에 `overflow-x-clip` 을 걸었다. `overflow-x: hidden` 이 아니라 `clip` 인 이유:
  **다른 축이 `clip` 이면 `overflow-y: visible` 이 `auto` 로 강등되지 않는다.** `hidden`
  이었으면 이긴 카드의 `scale(1.06)` 이 위아래로 잘린다.
- 대기 450ms → 680ms(연출 620ms + 한 박자). 동작 줄이기면 예전처럼 물러나기만 한다.

**② 우승 화면에 댓글.** 기존 `CommentSection` 을 그대로 붙였다.
**판(play)이 아니라 월드컵에 단다** — 판은 사람마다 매번 새로 생기므로 판별 댓글은 아무도
다시 안 본다. 월드컵 상세와 **같은 실**을 써야 대화가 쌓인다.

**③ 비로그인 댓글 — 판단이 필요했던 지점은 "본인 확인"이었다.**

요청에 명시된 입력란이 **닉네임과 내용 둘뿐**이라 한국 게시판의 관행인 **비밀번호 칸을 두지
않았다.** 대신 브라우저가 최초 1회 만들어 `localStorage` 에 두는 **작성자 토큰**을 쓰고,
서버는 **SHA-256 해시만** 저장한다. 한계는 명시해 둔다 — 브라우저 데이터를 지우거나 다른
기기·시크릿 창에서는 자기 댓글을 못 지운다. 비밀번호가 없는 이상 피할 수 없다.

**소유 판정을 서버로 옮겼다.** 예전엔 프론트가 `c.user.id === me?.id` 로 비교했는데 게스트
댓글에는 못 쓴다 — **프론트에는 평문 토큰만 있고 서버에는 해시만 있어서** 그 비교를 어느
쪽도 대신할 수 없다. 그래서 응답에 `is_mine` 을 넣었다. 이 때문에 `from_attributes` 로는
못 만들고 `_to_response()` 에서 명시적으로 조립한다.

**토큰을 보내는 방법이 요청 종류마다 다르다.** POST/PUT/DELETE 는 `X-Guest-Token` 헤더,
**목록(GET)은 쿼리 파라미터**다. `lib/api/client.ts` 가 GET 에 `Content-Type` 조차 일부러
안 붙이는 이유와 같다 — 커스텀 헤더가 하나라도 붙으면 단순 요청 조건이 깨져 **URL 마다 CORS
프리플라이트가 한 번씩 더 나간다**(2026-08-27에 없앤 문제다). POST 계열은 어차피
프리플라이트가 나가므로 헤더로 둬도 손해가 없다.

**중복 신고는 DB 가 막는다.** 로그인/비로그인 축이 달라 한 인덱스로 합칠 수 없어
**부분 유니크 인덱스 두 개**(`reporter_user_id IS NOT NULL` / `reporter_token_hash IS NOT
NULL`)로 나눴다. 걸리면 `IntegrityError` → 409 로 바꾼다.

`comments_author_present` 체크 제약으로 **주인 없는 댓글**(`user_id` 도 게스트 칸도 비어
있는 행)을 DB 에서 막았다. 라우터만 믿으면 나중에 다른 경로가 생겼을 때 조용히 샌다.

**수정(PUT)도 게스트에게 열었다.** 요청에는 삭제·신고만 있었지만 소유 판정 헬퍼가 삭제와
수정에서 같은 함수라, 수정만 로그인 전용으로 남기려면 **오히려 분기를 더 넣어야 했다.**

**신고 사유는 `DropdownMenu` 에서 바로 고른다.** `components/ui` 에 `Dialog` 프리미티브가
없는데 신고 하나 때문에 새로 들이지 않기로 했다 — `OwnerMenu` 가 쓰는 것과 같은 것을 쓴다.

**확인한 것.** Docker 가 떠 있어서 이번엔 **실제로 돌려 봤다.** 마이그레이션을 DB 에 적용해
`information_schema` 로 컬럼·인덱스·제약을 확인했고, uvicorn 을 띄워 curl 로 14가지 경로를
호출했다(비로그인 작성·토큰 없음 400·공백 400·중복 신고 409·자기 신고 400·남의 것 삭제
403·본인 삭제 204·`report_count` 가 보는 사람과 무관한지·삭제 시 신고 CASCADE·레거시
탑스터 경로 전 과정). **테스트 데이터는 전부 지웠다.** OpenAPI 와 `types/social.ts` 도
직접 비교해 필드·nullable 불일치 0건을 확인했다.

**못 한 것.** 프론트 화면은 여전히 눈으로 못 봤다. 상세 화면 본문이 통째로 스트리밍
Suspense 슬롯에 들어가서 **SSR HTML 에는 댓글 영역이 아예 안 나온다** — 다만 `좋아요`·
`이미지 저장` 도 똑같이 0건이라 **이번 변경 탓이 아니라 원래 그런 구조**다. `docs/TASKS.md`
에 확인 항목으로 남겼다.

### 2026-08-30 (계속) — 진행 계기판을 React Bits `SplitFlapText` 로 교체

**앞 항목의 "React Bits 에 split-flap 이 없다"는 틀렸다.** `SplitFlapText` 가 있다 —
"Mechanical split-flap departure board that clacks through to each new phrase", **의존성
없음**(`react` 만). 앞에서는 `FlipText`·`FlipCard`·`Counter`·`RotatingText` 만 찔러 보고
`SplitFlapText` 라는 이름을 안 넣은 채로 없다고 단정했다. 직접 만든 `FlapCounter` 내부를
버리고 이걸로 갈아 끼웠다.

**바꾸길 잘한 이유는 따로 있다.** 지금까지는 분자만 판이고 `/ 4` 는 맨 글자여서 **판
하나가 혼자 움직여 오히려 따로 놀았다.** `SplitFlapText` 의 `animateTo` 는 글자별로 비교해
**달라진 자리만** 넘긴다(`fromChar === targetChar` 면 계획에서 빠진다). `text="2/4"` →
`"3/4"` 를 주면 세 자리가 전부 같은 타일이면서 **앞 한 장만 움직인다** — 원하던 그대로다.
직접 만든 쪽은 이 비교 로직이 없어서 같은 걸 하려면 결국 다시 짜야 했다.

**그대로는 못 쓴다 — 세 가지가 막혔고 전부 실제로 확인했다.**

1. **`text` 하나만 주면 아무것도 안 넘어간다.** 원본의 구동부는 `words` 배열을 타이머로
   순환하는 것뿐이고, 문구가 하나면 `normalizedPhrases.length <= 1` 로 조기 반환한다.
   값을 바꿔도 타일이 그냥 갈아 끼워진다. **문구가 하나면 지금 값에서 새 값으로
   `animateTo` 를 한 번 돌리도록** 구동부를 바꿨다. 이게 이번 수정의 본체다.
2. **`pnpm lint` 가 깨진다.** effect 본문에서 `setTiles` 를 동기로 부른다
   (`SplitFlapText.tsx:174`). 이 프로젝트는 `react-hooks/set-state-in-effect` 가 error 라
   설치 직후 `npx eslint` 로 바로 재현됐다. 확정 경로를 `settleTo` 로 묶어 프레임 하나
   뒤(rAF 콜백)로 미뤘다 — 아래 `tick` 이 이미 같은 방식이라 결도 맞는다.
3. **`tsc` 가 깨진다.** `cycleTimerRef.current = window.setTimeout(...)` 이 `number` 를
   돌려주는데 ref 는 `ReturnType<typeof setTimeout>` 이다. `@types/node` 가 있는
   프로젝트에서는 그게 `Timeout` 이라 안 맞는다. 전역 `setTimeout` 으로 바꿨다.

**토큰도 맞췄다.** `tileColor`/`textColor` 기본값 `#111827`/`#f8fafc` → `var(--secondary)`/
`var(--foreground)`(CSS 안의 `var()` fallback 까지). 폰트는 전용 monospace 스택을 강제하는데
§ Typography 는 Geist 하나뿐이라 `inherit` 으로 바꿨다 — Geist 도 `tabular-nums` 가 있어
자릿수는 안 흔들리고, **기계 느낌은 폰트가 아니라 타일의 베벨·이음매·그림자가 낸다.**
`font-weight:760` 도 § Typography 에 없는 값이라 `700`(= `font-bold`)으로 내렸다.
`role="text"` 는 ARIA 명세에 없는 값(Safari 전용이었다)이라 `role="img"` 로 고쳤다.

**베벨·그림자의 흰색/검정 알파값은 그대로 뒀다.** 브랜드 색이 아니라 재질 음영이라
`--border`(`oklch(1 0 0 / 10%)`)와 같은 성격으로 본다.

**`flipsPerChar={0}`.** 원본 기본값 8 은 무작위 글자를 여덟 장 훑는 슬롯머신인데, 원한 건
값이 바뀔 때 한 번 "철컹" 걸리는 쪽이다. 훑는 맛을 원하면 3~4 로 올리면 된다.
`padTo={0}` 도 필요했다 — 기본값 12 는 빈 타일 아홉 장을 더 만든다.

**곁다리 소득**: 이 컴포넌트는 `prefers-reduced-motion` 을 **자체적으로 본다**(JS 훅 +
CSS 미디어 쿼리 양쪽). 이 화면의 다른 React Bits 연출과 달리 `useReducedMotion` 을 따로
걸지 않아도 된다. 직접 만들었던 `flap-top`/`flap-bottom` keyframes 는 `globals.css` 에서
지웠다(참조 0건 확인).

**확인한 것과 못 한 것.** `tsc` · `pnpm build` · eslint 통과(error 2건은 이번에도 건드리지
않은 기존 것) + 빌드 JS 청크에 `font-family:inherit` · `font-weight:700` ·
`var(--secondary)` · `var(--foreground)` 가 들어가고 `SFMono` · `#111827` 이 사라진 것까지.
**넘어가는 모습 자체는 여전히 눈으로만 판정된다** — 이 컴포넌트는 스타일을 `<style>` 태그로
직접 주입해서 Tailwind 번들에도 안 잡힌다.

### 2026-08-30 (계속) — 제목/진행도 분리, 진행도를 split-flap 계기판으로

**제목과 진행도를 세로로 나눴다.** 예전엔 `16강 1/4` 한 덩어리였다. **둘은 성격이 다른
값이다** — 제목은 라운드마다 귀해지는 것(바로 앞 항목의 `TITLE_RARITY`)이고, 진행도는 어느
라운드에서나 똑같이 읽혀야 하는 계기판이다. 한 줄에 붙여 두면 **제목에 건 광택이 숫자까지
훑어서** 그 구분이 사라진다. 이제 제목 `<p>` 에는 숫자가 없으므로 `tabular-nums` 도 아래로
옮겼다.

**진행도는 공항 안내판(split-flap)으로 넘어간다.** `components/tournament/FlapCounter.tsx`.

**~~React Bits 에는 split-flap 이 없다 — 직접 만들었다.~~ 틀렸다.** `SplitFlapText` 가
있다(바로 아래 항목에서 교체했다). 아래 후보 조사는 `FlipText`·`FlipCard`·`Counter`·
`RotatingText` 만 찔러 보고 **정작 `SplitFlapText` 라는 이름을 안 넣은 채로 "없다"고
단정한 것**이다. 아래 내용은 그 네 개에 대해서는 여전히 맞다:
`Counter` 는 `useSpring` 으로 숫자를 세로로 미끄러뜨리는 **오도미터**라 목표한 "철컹" 의
정반대고(게다가 `gradientFrom='black'` 하드코딩에 크기를 px 로 받아 토큰과 안 맞는다),
`RotatingText` 는 3D 로 뒤집기는 하지만 **문구를 자동 순환**하는 것이라 값 카운터가 아니다.
`FlipText`·`FlipCard` 는 아예 없다(레지스트리 404).

**만든 방식.** 글자를 잘라 그리는 게 아니라 **타일 높이 그대로의 글자 상자를 반 높이 상자에
넣고 위/아래로 붙여 넘치는 쪽을 잘라낸다**(`overflow-hidden`). 겹쳐 놓으면 정지 상태에서는
원래 글자와 똑같고 가운데 이음매만 남는다. 넘어갈 때는 위 절반에 새 값이 이미 떠 있고 아래
절반에 아직 이전 값이 남아 있는 상태에서 ① 이전 값의 윗장이 접히고 ② 새 값의 아랫장이
세워진 채 내려와 꽂힌다.

**"철컹" 은 easing 이 만든다.** `flap-bottom` 의 `cubic-bezier(0.2, 1.2, 0.35, 1)` 은 y2 가
1 을 넘어서 **0deg 를 살짝 지나쳤다가 되돌아온다.** 접히는 쪽은 `forwards` 로 세운 채 두고
(모서리만 보여 사실상 안 보인다), 내려오는 쪽은 `both` 여야 160ms 지연 동안 눕지 않는다.

**`key` 를 갈아 끼워야 다시 돈다.** 같은 엘리먼트에 클래스만 다시 붙이면 브라우저가 이미
끝난 애니메이션으로 보고 재생하지 않는다. `key={current}` 로 두 장을 통째로 갈아 끼운다.

**이전 값을 effect 없이 따라간다.** 아래 절반에 남겨 둘 이전 값이 필요한데, 이 프로젝트는
`react-hooks/set-state-in-effect` 가 error 라 effect 로 prop 을 따라가게 만들 수 없다.
React 가 "prop 이 바뀔 때 state 를 조정하는" 용도로 문서화해 둔 **렌더 중 setState** 패턴을
썼다(조건 안에 있어 루프가 되지 않는다). eslint 통과를 확인했다.

**결승에는 계기판을 걸지 않는다.** `1/1` 은 정보가 아니다. 분모는 라운드 안에서 변하지
않으므로 넘어가는 것은 분자 하나뿐이다.

**확인한 것과 못 한 것.** `tsc` · `pnpm build` · eslint 통과(error 2건은 이번에도 건드리지
않은 기존 것) + 빌드 CSS 에 `animate-flap-{top,bottom}` 과 `@keyframes` 가 들어간 것,
`origin-top`/`origin-bottom`/`h-1/2`/`min-w-8` 유틸리티가 실제로 생성된 것까지.
**걸리는 느낌이 실제로 나는지는 눈으로만 판정된다** — 안 읽히면 y2 를 키우거나 260ms 를
늘려야 한다. `docs/TASKS.md` 에 남겼다.

### 2026-08-30 (계속) — 부제 삭제, 제목에 라운드별 희귀도

**부제를 뺐다.** 준결승·결승 제목 아래에 붙던 `이기는 쪽이 우승합니다` / `이기는 쪽이 결승에
오릅니다` 를 삭제했다. **거슬린다는 지적을 받았고, 맞는 지적이다** — 판이 끝나 간다는 건
제목("결승")과 배경 대진표의 '우승' 자리가 이미 말하고 있는데 같은 말을 문장으로 한 번 더
한 것이었다. 이 한 줄만 쓰던 React Bits `BlurText` 도 같이 지웠다(파일째).

이건 바로 앞 항목에서 세운 근거("이 두 라운드에서만 '이기면 어디로 가는지'가 확정된
정보다")를 **뒤집은 것**이다. 근거 자체는 틀리지 않았지만, 그 정보가 이미 화면에 두 번
나와 있다는 걸 계산에 안 넣었다.

**제목에 희귀도를 넣었다.** 결승 하나만 달라 보이던 것을 **32강 → 결승 5단계**로 벌렸다.
결승의 연출은 마음에 든다는 말을 들었으므로 **결승 값은 그대로 두고 아래로만 내렸다** —
천장을 고정하고 사다리를 만든 것이다.

| 라운드 | `round_num` | 광택 밝기 | 주기 |
|---|---|---|---|
| 32강 | 5 | 35% | 6s + 4s 쉼 |
| 16강 | 4 | 55% | 5s + 3s 쉼 |
| 8강 | 3 | 75% | 4s + 2.5s 쉼 |
...
**`round_num` 은 경기 수가 아니라 지수다.** `roundLabel()` 이 `2 ** roundNum` 으로 이름을
만든다(5 → `32강`, 3 → `8강`). 1·2 만 결승·준결승 예외. 표를 짜기 전에 이걸 확인하지 않았으면
`8: 16강` 같은 식으로 통째로 어긋났을 것이다.

**64강·128강(6·7)은 표에 없어 광택이 아예 없다.** 아직 귀할 단계가 아니고, 128강부터
번쩍이면 결승까지 갈 곳이 없다.

**등급을 색상으로 안 나눈 이유.** 게임식 희귀도는 보통 색상환(회색→녹색→파랑→보라→금색)으로
나누는데 **여기서는 못 쓴다** — § Color budget 이 saturated 토큰을 `primary`(amber)와
`destructive` 둘로 제한하고, 이 화면은 선택 버튼 두 개로 이미 WARN 선이다. 대신
§ Visual reference 가 다크 UI 에 대해 말하는 축을 썼다: **"캔버스가 검을수록 그림자보다 밝기
단계로 위계를 표현한다."** 그래서 축이 색상이 아니라 **밝기 + 주기**다.

**밝기 단계는 `color-mix()` 로 만든다.** `color-mix(in oklch, var(--foreground) 55%,
var(--muted-foreground))` 처럼 두 토큰 사이를 섞는다. hex 를 다섯 개 새로 만들지 않으므로
§ Color 하드코딩 금지에 걸리지 않고 테마를 따라간다. `ShinyText` 가 받은 색 문자열을 그대로
`linear-gradient()` 에 넣기 때문에 색 함수도 변수와 똑같이 동작한다.

**`key={match.round_num}` 이 필요하다.** 없으면 컴포넌트가 재사용되면서 32강의 느린 광택
주기를 물려받은 채로 결승에 들어간다.

**확인한 것과 못 한 것.** `tsc` · `pnpm build` · eslint 통과(error 2건은 이번에도 건드리지 않은
기존 것). **사다리가 단계로 느껴지는지는 32강부터 한 판을 끝까지 돌려 봐야 안다** — 한 라운드만
보면 옆 단계와 비교가 안 된다. `color-mix` 가 브라우저에서 계산되는지도 런타임 문자열이라
빌드로 검증되지 않는다(실패하면 그라디언트가 무효가 되어 **글자가 통째로 안 보인다**).
둘 다 `docs/TASKS.md` 에 남겼다.

### 2026-08-30 (계속) — 준결승·결승에 React Bits

**왜 다시 손댔나.** 같은 날 앞 항목에서 만든 준결승·결승 연출이 **밋밋하다는 지적을 받았다.**
원인은 분명했다 — 그때 늘린 건 정보(캡션 한 줄)와 글자 크기(`text-2xl` → `text-3xl`)뿐이고
**감각이 하나도 안 바뀌었다.** 결승이 결승처럼 느껴지려면 방의 조명이 바뀌어야 하는데
글자 크기는 조명이 아니다. 색을 못 쓰는 제약(§ Color budget 의 primary 2개를 선택 버튼이
이미 다 쓴다)은 그대로여서, **색 말고 다른 축 — 움직임 — 을 쓰기로 했다.**

**React Bits 를 고른 경로.** 별도 CLI 가 필요 없다. 레지스트리 항목이 shadcn 스키마
(`ui.shadcn.com/schema/registry-item.json`)라 **이미 쓰고 있는 shadcn CLI 로 그대로 붙는다.**

```
pnpm dlx shadcn@latest add https://reactbits.dev/r/StarBorder-TS-TW.json
```

URL 규칙은 `/r/{이름}-{JS|TS}-{CSS|TW}.json`. 이 프로젝트는 `tsx: true` + Tailwind v4 라
`-TS-TW`. 패키지 의존이 아니라 **소스가 복사되는 방식**이라 붙인 뒤 토큰에 맞게 고칠 수 있고,
실제로 고쳐야 했다(아래).

**`pnpm` 이다 — `npm install` 이 깨진다.** `npm install motion` 이
`Cannot read properties of null (reading 'matches')` 로 죽는다. `node_modules` 가 pnpm 의
심링크 배치라 npm arborist 가 `node_modules/.pnpm/...` 를 못 읽는 것이다. **`pnpm-lock.yaml`
이 정본이므로 `pnpm add` 를 쓴다.** (CLAUDE.md 의 로컬 실행 절은 `npm run dev` 라고 적혀
있는데, 스크립트 실행은 npm 으로도 되지만 **설치는 안 된다.**)

**고른 것과 고르지 않은 것.** 레지스트리 JSON 을 직접 받아 의존성을 확인하고 골랐다.

| 쓴 것 | 의존성 | 자리 |
|---|---|---|
| `StarBorder` | **없음**(CSS keyframes 만) | 결승 카드 두 장의 테두리 |
| `ShinyText` | `motion` | 결승 라벨 |
| `BlurText` | `motion` | 준결승·결승 캡션 |
| `CountUp` | `motion` | 우승 화면의 우승 비율·승률 |

**`SpotlightCard`·`GlareHover`·`TiltedCard` 를 뺀 이유는 전부 커서를 따라가기 때문이다 —
월드컵은 대부분 휴대폰에서 돌린다.** 마우스가 필요한 연출은 거기서 아무 일도 안 일어난다.
`StarBorder` 는 입력과 무관하게 도는 것이라 모바일에서도 산다. `SplitText` 는 `gsap` +
`@gsap/react` + `SplitText` 플러그인까지 끌고 와서 라벨 한 줄에 쓸 무게가 아니고,
`Aurora`·`Particles` 는 `ogl`(WebGL) + `<canvas>` rAF 상시 구동이라 **대결이 진행되는
화면에는 안 맞는다**(우승 화면처럼 종료 상태라면 다시 볼 만하다).

**받은 소스를 그대로 쓰면 안 된다 — DESIGN.md 위반이 박혀 있다.** `StarBorder` 는
`rounded-[20px]`(§ Radius 의 5종 밖 → WARN), `backgroundColor:'#000000'`,
`borderColor:'#222222'`(§ Color 하드코딩 금지)를 안쪽 div 에 직접 칠한다. 이건 원본이 **그
자체로 버튼**이라 표면을 스스로 그리기 때문인데, 여기서는 `Card` 를 감싸는 껍데기다.
그래서 **표면을 그리던 div 를 통째로 뺐다** — 배경·테두리·여백은 감싸이는 `Card`(`bg-card`,
`ring-1 ring-foreground/10`, `rounded-xl`)가 갖는다. 두 겹으로 칠하면 하드코딩된 검정이
토큰을 덮어써 다크 테마가 어긋난다. 기본 색도 `white` → `var(--foreground)` 로 바꿨다.

**색은 이번에도 하나도 안 늘렸다.** `ShinyText` 에 hex 대신 **CSS 변수를 넘길 수 있다는 점**이
결정적이었다 — 이 값들은 그대로 `linear-gradient()` 문자열에 들어가므로
`color="var(--muted-foreground)"` / `shineColor="var(--foreground)"` 가 그대로 동작한다.
무채색 광택이라 § Color budget 의 primary 카운트는 선택 버튼 2개 그대로다.

**keyframes 는 `globals.css` 에.** 원본은 `tailwind.config.js` 의 `theme.extend` 에 넣으라는
주석을 준다. **이 프로젝트에는 config 파일이 없다**(Tailwind v4, `components.json` 의
`"config": ""`). v4 에서는 `@theme` 안의 `--animate-*` 가 곧 `animate-*` 유틸리티가 되므로
거기에 뒀다. 빌드 CSS 에서 유틸리티 2개와 `@keyframes` 2개가 실제로 나온 것을 확인했다
(`.animate-star-movement-top{animation:var(--animate-star-movement-top)}`).

**`prefers-reduced-motion` 은 우리가 처리해야 한다 — React Bits 는 어디서도 안 본다.**
`lib/hooks/use-reduced-motion.ts` 를 만들어 넷 다 정적 마크업으로 갈아끼운다.
`useState`+`useEffect` 가 아니라 **`useSyncExternalStore`** 인 이유는 두 가지다:
① 이 프로젝트는 `react-hooks/set-state-in-effect` 가 error 라 effect 에서 초기값을 못 넣는다
② 서버에 `matchMedia` 가 없어 SSR 스냅샷이 따로 필요한데 `useSyncExternalStore` 가
hydration mismatch 없이 이걸 처리해 준다. 기존에 `vote()` 안에서 인라인으로 읽던
`window.matchMedia(...)` 도 이 훅으로 합쳤다.

**준결승과 결승을 다르게 뒀다.** 테두리(`StarBorder`)는 **결승에만** 두른다. 준결승은
라벨·캡션 연출까지만 — 둘을 똑같이 꾸미면 "결승이 마지막"이라는 위계가 사라진다.
이건 앞 항목에서 세운 원칙("이 화면에서 움직이는 것은 카드 전환 하나여야 한다")을
**결승 한 라운드에 한해 의도적으로 깬 것**이다. 초반 라운드는 그대로 두었으므로 원칙 자체는
살아 있다.

**확인한 것과 못 한 것.** `tsc` · `pnpm build` · eslint 통과(남은 error 2건은 이번에 건드리지
않은 `app/search/page.tsx`·`components/layout/Navbar.tsx` 의 기존 것이다) + 빌드 CSS 에
keyframes 가 들어간 것까지. **애니메이션 자체는 원리상 이걸로 검증이 안 된다** — Docker 가
꺼져 있어 스택을 못 띄웠다. 특히 **`opacity-40` 배경 대진표와 `StarBorder` 가 겹쳐 지저분해지지
않는지**는 눈으로만 판단할 수 있다. `docs/TASKS.md` 에 확인 항목으로 남겼다.
...
매 경기 같은 문장을 읽는 것보다 **지금 몇 강의 몇 번째인지**가 쓸모 있다 — 128강이면
그게 없으면 끝이 안 보인다. 화면 한가운데 `8강 1/4` 을 `text-2xl` 로 놓았다.
라운드 안에서의 순번은 `match_num` 이 0부터인지 1부터인지에 기대지 않고 **정렬된 자리로**
센다. 헤더 왼쪽에 있던 `준결승 · 남은 경기 N개` 는 지웠다 — 가운데 표기와 겹친다.

**② 커버에서 투표를 떼어 내고 '선택' 버튼을 아래에 뒀다**
예전엔 커버를 누르는 것이 곧 투표라서 미리듣기를 그 위에 얹을 수 없었다(버튼 안의 버튼).
투표를 카드 하단 버튼으로 내리면서 재생 버튼이 후보 그리드 타일과 **같은 자리**(커버 우측
상단)로 왔다. 커버 옆을 잘못 눌러 투표되는 일도 없어진다.
선택 버튼 둘은 primary 다 — 화면의 목적 자체가 둘 중 하나를 고르는 것이라
DESIGN.md § Color budget 의 WARN 선(2개)까지 쓴다.

**③ 우승 화면에 커버와 누적 성적**
우승 항목의 커버를 `max-w-xs` 로 크게 놓고 **우승 비율·승률**을 카드 두 장으로 붙였다.
값은 `GET /tournaments/{id}/ranking` 의 그 항목 한 줄에서 꺼낸다 — 이 판 한 번의 결과가
아니라 **월드컵 전체 누적**이고, 마지막 투표가 기록된 뒤에 부르므로 방금 끝난 판까지
반영돼 있다. 실패해도 우승 자체는 그대로 보여 준다(성적 칸만 빠진다).

**검증**
- `tsc --noEmit`·`next build` 통과, eslint 신규 0건(전체 2건은 기존)
- **API로 한 판을 끝까지 돌려 봤다** — 4강 판에서 화면 표기가 `준결승 1/2` → `준결승 2/2`
  → `결승 1/1` 로 나온다(8강 판이면 `8강 1/4`). 우승 후 랭킹 행도 확인:
  `우승 비율 9%(1/11판) · 승률 57%(4/7경기)`
- 진행 중·완료된 플레이 화면 모두 200
- **화면 자체는 눈으로 못 봤다** — 이 페이지는 데이터를 `useEffect` 에서 받아서 SSR HTML 에
  대결 카드가 없다(TASKS 의 '플레이 화면 확인' 항목에 그대로 남는다)
...
**⑦ 뒤이어 — 앨범 후보에 라우팅을 같이 붙였다**
"후보·랭킹에서 앨범을 누르면 재생도 앨범 페이지도 갈 수 있게" — 재생 버튼 옆에
`/albums/{id}` 로 가는 버튼을 나란히 둔다(앨범 타입일 때만). **타일 전체를 링크로
덮는 방법(앨범 카드가 쓰는 패턴)은 안 썼다** — 후보 그리드는 지금까지 아무 데도
가지 않던 타일이라 보이지 않는 전체 링크가 놀라움이 되고, 같은 `PoolItemTile` 이
편집기에서는 '빼기' `<button>` 으로 쓰여다 — 버튼 안에 링크를 넣을 수 없다.

**버튼 색은 중립을 유지했다.** "hover 때만 띄우면 primary 로 해도 되지 않느냐"는
지적이 있었고 데스크톱에서는 맞는 말이다(한 번에 하나만 보인다). 다만 **hover 가 없는
모바일에서는 그리드의 버튼이 전부 동시에 보인다**(2열 × 3행 = 6개). 그게 이유라고
확인받아 현행 중립색을 그대로 둔다.
...
- 페이지 12종 200 — 홈·검색·목록 2·만들기 2·프로필·로그인·앨범 상세·아티스트 상세·
  월드컵 상세·랭킹·플레이
- 앨범 월드컵은 후보·랭킹 양쪽에 재생 8·앨범보기 8, 곡 월드컵은 재생 20·앨범보기 0
  (곡 후보는 갈 곳이 없어 링크를 안 달았다). 그 링크를 따라간 `/albums/1097864180` 도 200
