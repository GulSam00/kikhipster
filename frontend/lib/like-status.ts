...
async function runBatch(type: LikeTargetType, ids: string[]): Promise<void> {
  for (let i = 0; i < ids.length; i += BATCH_LIMIT) {
    ...
    try {
      const res = await apiFetch<Record<string, LikeStatus>>(
        `/api/likes/batch/${type}?ids=${chunk.map(encodeURIComponent).join(',')}`,
      );
...
export function useLikeStatus(type: LikeTargetType, id: string): UseLikeStatus {
  ...
  const toggle = useCallback(async () => {
    ...
    wakeAll();
    try {
      statuses.set(k, await apiFetch<LikeStatus>(`/api/likes/${type}/${id}`, { method: 'POST' }));
    } catch (err) {
      statuses.set(k, prev);
      throw err;
    } finally {
